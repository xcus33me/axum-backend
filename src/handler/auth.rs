...
pub async fn verify_email(
    ...
    if let Some(expires_at) = user.token_expires_at {
        if Utc::now() > expires_at {
            return Err(HttpError::bad_request(ErrorMessage::VerifTokenExpired.to_string()))?;
        }
    } else {
        return Err(HttpError::bad_request(ErrorMessage::InvalidVerifToken.to_string()))?;
    }

    app_state.db_client.verify_token(&query_params.token).await
        .map_err(|e| HttpError::server_error(e.to_string()))?;
...
pub async fn forgot_password(
    ...
    let vertification_token = uuid::Uuid::new_v4().to_string();
    let expires_at = Utc::now() + Duration::minutes(30);

    let user_id = uuid::Uuid::parse_str(&user.id.to_string()).unwrap();

    app_state.db_client
        .add_verified_token(user_id, &vertification_token, expires_at)
        .await
        .map_err(|e| HttpError::server_error(e.to_string()))?;
    ...
    if let Err(e) = sent_email_result {
